...
- **.well-known이 Git에 안 올라갈 때**: 이 폴더에는 `!.well-known/` 규칙이 들어 있는 `.gitignore`가 있어, dot 폴더가 무시되지 않도록 했습니다. 그래도 커밋이 안 되면 저장소 루트에서 `git add .well-known/assetlinks.json` 또는 `git add ".well-known/"` 로 직접 추가한 뒤 커밋해 보세요.
